...
void loop() {
  ...
  if (skip_load > 0) {
    ...
    if ((motionlevel[0] > 0 || motionlevel[1] > 0 || motionlevel[2] > 0) && (motion_warn > 7)) {  //2단계 8+
      digitalWrite(34, LOW);
      analogWrite(A15, 255);
      delay(1000);
      analogWrite(A15, 0);
      ...
      digitalWrite(34, HIGH);
      skip_load = 1;
    } else if ((motionlevel[0] > 0 || motionlevel[1] > 0 || motionlevel[2] > 0) && (motion_warn < 8) && (motion_warn > 4)) {  //1단계 5,6,7
  ...
  while (1) {
    ...
    if ((hic_print < 10) && (SmartAllet_toolowtemp == 0)) {  //온도 너무낮음
      SmartAllet_toolowtemp = 1;                             //SmartAllet 실행여부기록
      dfmp3.playMp3FolderTrack(15);
      delay(4000);
    } else if ((hic_print > 28) && (SmartAllet_toohightemp == 0)) {  //온도 너무높음
      SmartAllet_toohightemp = 1;                                    //SmartAllet 실행여부기록
      dfmp3.playMp3FolderTrack(16);
      delay(4000);
    } else if (SmartAllet_SecurityAllet == 1) {  //보안시스템 작동
    ...
    if (bat_level_k < bat_pst_current) {
      if ((bat_pst_current - bat_level_k)>5) {
        bat_val_torque = bat_val_torque + 5;
      }
      else {
        if (bat_val_torque > 0) {
          bat_val_torque = bat_val_torque - 5;          
    ...
    if ((sensor == 1) && (adas == 1)) {                        //ADAS모듈이 정상작동중이라면
      if ((b1 < 30) || (b2 < 30) || (b3 < 30) || (b4 < 30)) {  //ADAS1 긴급경고(주변물체 너무가까움)
        if (adas_info_crt == 0) {
          adas_info_crt = 1;  //오류값으로 ADAS 긴급경보 발동방지를 위해 최초1회는 무시
        } else {
      ...
      } else if ((b1 < 60) || (b2 < 60) || (b3 < 60) || (b4 < 60)) {  //기본경고
    ...
    if ((AdditionalSoundCount > 600) && (adas == 1)) {
      dfmp3.playMp3FolderTrack(1);
      delay(50);
